package com.bolsadeideas.springboot.webflux1.app.controller;
...
public class ProductoController {
	...
	public Flux<Categorias> categorias() {
		return service.findAllCategoria();
	}
	
	@GetMapping("/listar")
	public String listar(Model model) {
		Flux<Producto> productos = service.returnAllWhitUppercaseInFieldNombre();
		
		productos.subscribe(prod -> log.info(prod.getNombre()));
		
		model.addAttribute("propertiProducts",productos);
	...
	public String listarDataDriver(Model model) {
		...
		Flux<Producto> productos = service.returnAllWhitUppercaseInFieldNombre().delayElements(Duration.ofSeconds(1));

		productos.subscribe(prod -> log.info(prod.getNombre()));

		model.addAttribute("propertiProducts", new ReactiveDataDriverContextVariable(productos, 1));
		model.addAttribute("titulo", "Listado de productos");
		return "listar";
	}
	...
	public Mono<String> guardar(@Valid Producto producto, BindingResult result, Model model,@RequestPart(name = "file") FilePart archivo, SessionStatus status) {
		if (result.hasErrors()) {
			...
			Mono<Categorias> categoria = service.findByIdCategorias(producto.getCategorias().getId());
			...
			return categoria.flatMap(c -> {
				...
				log.info("Categoria asignada: " + p.getCategorias().getNombre() + " Id Cat: "
						+ p.getCategorias().getId());
	...
	public Mono<String> eliminar(@PathVariable(name = "id") String id) {
	...
	@GetMapping("/ver/{id}")
	public Mono<String> ver(Model model, @PathVariable(name = "id") String id){
